...
def num_points_scored(name)
  if game_hash[:away][:players].keys.include?(name)
    game_hash[:away][:players][name][:points]
  elsif game_hash[:home][:players].keys.include?(name)
...
def team_colors(name)
  if game_hash[:away][:team_name].include?(name)
    game_hash[:away][:colors]
  elsif game_hash[:home][:team_name].include?(name)
...
def player_numbers(team)
  ...
  elsif game_hash[:home][:team_name].include?(team)
  ...
  rebound = 0
    game_hash.each do |side, team|
      team[:players].each do |name, details|
        if details[:shoe] > biggest_shoe
          biggest_shoe = details[:shoe]
         rebound = details[:rebounds]
         end
    end
  end
  rebound
